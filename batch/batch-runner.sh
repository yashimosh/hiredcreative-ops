...
# hiredcreative-ops batch runner — standalone orchestrator for claude -p workers
...
usage() {
  cat <<'USAGE'
hiredcreative-ops batch runner — process job offers in batch via claude -p workers
...
main() {
  ...
  echo "=== hiredcreative-ops batch runner ==="
  ...
  if (( PARALLEL <= 1 )); then
    ...
    for i in "${!pending_ids[@]}"; do
      ...
      while (( running >= PARALLEL )); do
        ...
        # Compact arrays (guard against empty array with nounset)
        pids=("${pids[@]+"${pids[@]}"}")
