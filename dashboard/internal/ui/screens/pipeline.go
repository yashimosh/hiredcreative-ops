		...
		if app, ok := m.CurrentApp(); ok {
			...
			return m, func() tea.Msg {
				return PipelineUpdateStatusMsg{
					HiredCreativeOpsPath: m.careerOpsPath,
					App:           app,
					NewStatus:     newStatus,
				}
			}
...
func (m PipelineModel) loadCurrentReport() tea.Cmd {
	...
	return func() tea.Msg {
		return PipelineLoadReportMsg{HiredCreativeOpsPath: path, ReportPath: report}
	}
}
...
func (m PipelineModel) renderHelp() string {
	...
	brand := lipgloss.NewStyle().Foreground(m.theme.Overlay).Render("hiredcreative-ops by hiredcreative.com")
