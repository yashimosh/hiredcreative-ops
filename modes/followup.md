...
**Method layer:** `library/outreach.md` §cadence (every touch adds NEW value — never "bumping this"; break-up note at the end of the sequence). Two upgrades from the deep sources: **kairos beats calendar** — a behavioral signal (reply, profile view, company news) overrides the fixed day-count as the trigger to send (`library/custom/books/persuasive-technology.md`); **name yellow lights instead of surviving them** — vague "still deciding" replies get a direct, warm "I have a concern — is timing the real issue here?" rather than another silent wait cycle (`library/custom/books/lets-get-real.md`). Dead threads get the one-sentence reviver from `modes/negotiation.md` §7.
...
> "No active applications to follow up on. Apply to some roles first with `/hiredcreative-ops` and come back when they're aging."
...
- Suggest the user run `/hiredcreative-ops contacto {company}` to find the right person first
...
> - Trying a different contact via `/hiredcreative-ops contacto`
...
**To:** {email or "No contact found — run `/hiredcreative-ops contacto` first"}
