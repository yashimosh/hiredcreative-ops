...
**Method layer:** an interview is a sales conversation — `library/sales-conversations.md` applies wholesale. Key adaptations: make the interviewer name the gap before answering it (a "benefit" only counts against a need THEY stated — `library/custom/books/spin-selling.md`); STAR+R answers = low-context Western register, point first then evidence (`library/communication.md`); prep from the interviewer's chair (Carnegie P3.8); end every round with an advance — a defined next step, not "hope to hear from you."
