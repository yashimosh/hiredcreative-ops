...
5. **hiredcreative-ops proof point**: Include in "Additional info" if there is a field for it
...
3. Suggest next step: `/hiredcreative-ops contacto` for LinkedIn outreach
...
## Email applications (no ATS form)

When the company asks to apply by email (e.g. "send your CV to hr@company.com"):

### Sending method — use a transactional email API, not a local SMTP client

A plain HTTPS email API (e.g. Resend, Postmark, SES) is more reliable in an
unattended/headless flow than a local mail client's SMTP bridge — no session
dependency, and delivery is confirmed by the API response.

- Store the API key in your password manager / secrets store — never hardcode it in a mode file.
- Send from a domain you've verified with the provider (see `config/profile.yml` for your sending address).
- Confirm delivery via the provider's dashboard or API response, and log the message ID in the pipeline entry.

### Workflow

1. Generate tailored CV HTML → PDF via `generate-pdf.mjs`
2. Generate cover letter HTML → PDF via `generate-pdf.mjs`
3. Save both to `output/{slug}/`
4. **SHOW THE USER BOTH PDFs BEFORE SENDING** — use `SendUserFile` and wait for explicit approval
5. Send via Resend API with both PDFs as base64 attachments
6. Log Resend email ID in pipeline entry and tracker

### Email format

```
From: {Your Name} <{your verified sending address}>
To: {hr email}
Subject: Application: {Role Title}
Body: brief intro (3-4 sentences max), portfolio link, sign-off
Attachments: resume PDF + cover letter PDF
```

### MANDATORY: Review before send

Never send without the user seeing the documents first. Show PDFs via `SendUserFile`, confirm, then send.
