...
## Weekly Full Sweep — primary execution model

Scans run **once a week**. There is no daily rotation and no due/not-due gating —
**every tile in `data/scan-rotation.yml` runs in every sweep.** The manifest defines
WHAT to cover (tiles) and in what ORDER (waves); this file defines HOW.

### Track-filtered scan (on-demand, any session)

If invoked as `scan remote` or `scan relocation` instead of bare `scan`, run an ad-hoc
scan scoped to one track instead of the full weekly sweep. This does NOT replace the
weekly full sweep or touch its `last_scanned` cadence — it's a targeted top-up any
session can run mid-week.

1. Filter tiles from `data/scan-rotation.yml` by the requested track:
   - `remote` → tiles with `track: remote`, `track: both`, or no `track:` field (default)
   - `relocation` → tiles with `track: relocation` or `track: both`
2. Run only the matching tiles through waves 1–4 (skip waves that have no matching tiles).
3. Triage & close (wave 6) as normal, but the sweep report header reads
   `Track Scan ({track}) — {YYYY-MM-DD}` instead of `Weekly Sweep`, and the coverage
   ledger lists only the filtered tile set (not all 56).
4. Still stamp `last_scanned` on every tile that ran — this feeds the next weekly
   sweep's freshness math, so an ad-hoc track scan reduces (not duplicates) work in
   the following full sweep.

### Wave plan

| Wave | What | Parallelism |
|------|------|-------------|
| **0. Prep** | Load dedup sources (`scan-history.tsv`, `applications.md`, `pipeline.md`, track files). Check `cv-sync-check.mjs`. | — |
| **1. Zero-token APIs** | `node scan.mjs` (Greenhouse/Ashby/Lever/Workable/BambooHR + Hired Creative) + Teamtailor RSS + Workday script | One command; runs while Wave 2 dispatches |
| **2. WebSearch families** | All `websearch-*` + `*-direct` tiles, all `search_queries` in `portals.yml` | Parallel subagents, one per tile family |
| **3. Authenticated surfaces** | LinkedIn (saved/notifications/recommendations/searches), Instagram feeds, Behance, Wellfound, Dribbble, YC, email triage | Chrome MCP = ONE browser → sequential within the wave |
| **4. Playwright company sweeps** | All `tracked_companies` tiers + creative boards | Parallel agents, batches of 3–5 |
| **5. Triage & close** | Liveness-verify search-sourced URLs, dedup, track-tag, write pipeline files, stamp `last_scanned`, sweep report | Sequential |

Dispatch waves 1, 2, 4 as background subagents in parallel; run wave 3 in the main
session (browser is a serial resource). Wave 5 only starts when all others report.

### Freshness window

Because sweeps are weekly, every time-filtered search uses a **10-day window**
("past week" LinkedIn filter, `posted within 10 days` judgment on boards) — 7-day
cadence + 3-day margin so a posting can never fall in the gap between two sweeps.
Social feeds (Instagram, FB, Telegram): read back to the previous sweep's
`last_scanned` date, not a fixed post count.

### Completeness contract

A sweep is **not done** until every tile reports one of:
- `done` — ran, N candidates found (0 is a valid result)
- `blocked({reason})` — auth expired, 403, site down… retried ONCE, then surfaced

Rules:
1. **No silent skips.** Blocked tiles go in the sweep report under "Needs manual follow-up" with the exact reason and URL.
2. **Retry once** on transient failure (timeout, 5xx). Two failures = blocked, move on.
3. **Coverage ledger**: the sweep report lists EVERY tile id with its status and count. If a tile id from the manifest is missing from the ledger, the sweep is incomplete — go back.
4. **Dead config surfaces**: careers_url 404s, dead query patterns, changed page slugs → listed in the report AND fixed in `portals.yml`/manifest in the same session.
5. Stamp `last_scanned: {ISO date}` on every tile that ran (done OR blocked).

### Dedup — three layers, in order

1. `scan-history.tsv` — exact URL seen before
2. `pipeline.md` + track files — URL already queued/processed
3. **Fuzzy company+role vs `applications.md`** — normalize company name + role title;
   if an Applied/Rejected/SKIP entry matches, mark `skipped_dup`. NEVER present a
   highlight without this check (lesson 2026-06-28: scan.mjs only dedups exact URLs —
   Anthropic + Sardar resurfaced as "fresh" despite being already actioned).

### Sweep report (end of every weekly scan)

```
Weekly Sweep — {YYYY-MM-DD}
━━━━━━━━━━━━━━━━━━━━━━━━━━━
Coverage: {done}/{total} tiles done, {blocked} blocked
New candidates: {n} → remote {n} | relocation {n} | US bucket {n}
Filtered: {title} title / {dup} dup / {expired} expired

Highlights (fuzzy-dedup verified):
  + {company} | {title} | {track} | {tile}

Needs manual follow-up:
  ! {tile}: {reason}

Config fixes applied: {list or none}
→ Run /hiredcreative-ops pipeline to evaluate the new candidates.
```

Log one summary line to `data/daily-runs.log` (historical name — now weekly entries).

### Ejecución como subagentes

Cada wave paralelizable se ejecuta como subagente para no consumir contexto del main:

```
Agent(
    subagent_type="general-purpose",
    prompt="[wave/tile instructions from this file + portals.yml slice + dedup snapshot]",
...
## Track tagging (scan-time)

Every query/company in `portals.yml` MAY carry a `track:` field (`remote` | `relocation`).
When adding a new offer to `pipeline.md`, append the track hint from its source:

```
- [ ] {url} | {company} | {title} | track:{remote|relocation}
```

- Queries without a `track:` field default to `track:remote` (the "remote" keyword queries).
- The city-specific queries (London/Amsterdam/Berlin/Helsinki, "visa sponsorship") are `track:relocation`.
- The hint is advisory — final track assignment happens at evaluation time per `modes/_profile.md`.

**Scope boundary:** this mode scans the INTERNATIONAL tracks (remote + relocation). A
fully local/in-country employment mode is out of scope here but follows the same
pattern — see `modes/scan.md`'s wave structure for how to add one.
...
→ Ejecuta /hiredcreative-ops pipeline para evaluar las nuevas ofertas.
```
