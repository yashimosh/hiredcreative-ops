 ...
 *
 * Run: node hiredcreative-ops/merge-tracker.mjs [--dry-run] [--verify]
 */
...
// Schema-aware parse. We ONLY match/update within the evaluated table — the
// email-confirmed table (## Email-confirmed submissions) and the geo-blocked
// outcome table have different schemas and their own ID space. Matching an
// addition's number against an email-confirmed row and rewriting it with the
// evaluated schema is the data-loss bug this guards against (2026-06-17).
const parsed = parseApplicationsTables(appContent);
const existingApps = parsed.evaluated.filter(e => e.num != null);

// maxNum and the collision set span EVERY table in the file, so a new
// evaluated entry never reuses (and clobbers) an email-confirmed ID.
const existingIds = new Set(parsed.allIds);
let maxNum = parsed.maxId;

console.log(`📊 Existing: ${existingApps.length} evaluated entries, max #${maxNum} (all tables)`);
...
for (const file of tsvFiles) {
  ...
  if (duplicate) {
    ...
    // New entry. Entry number must equal the report number — the report
    // filename is the durable artifact, so it wins when the TSV disagrees
    // (guards against parallel workers self-assigning inflated IDs like the
    // 2026-06-22 batch, which scattered entries across 1000s/2000s bands
    // while reports were numbered independently).
    let entryNum = addition.num;
    const reportNumMatch = (addition.report || '').match(/reports\/0*(\d+)-/);
    if (reportNumMatch) {
      const reportNum = parseInt(reportNumMatch[1]);
      if (reportNum !== entryNum && !existingIds.has(reportNum)) {
        console.warn(`⚠️  ${addition.company}: TSV num #${entryNum} ≠ report num #${reportNum} — using report num`);
        entryNum = reportNum;
      }
    }
    // Collision fallback: next number above the global max across EVERY table
    // so a new evaluated entry never clobbers an email-confirmed row.
    if (!entryNum || existingIds.has(entryNum)) {
      entryNum = ++maxNum;
    }
    existingIds.add(entryNum);
...
// Insert new lines just under the evaluated table's header separator.
// In-place updates above don't shift line indices, so the separator index
// from the initial parse is still valid here.
if (newLines.length > 0) {
  let insertIdx = parsed.separatorIndex.evaluated;
  if (insertIdx == null) {
    // Fallback: first markdown table separator row in the file.
    for (let i = 0; i < appLines.length; i++) {
      if (appLines[i].includes('---') && appLines[i].startsWith('|')) { insertIdx = i; break; }
    }
  }
  if (insertIdx != null && insertIdx >= 0) {
    appLines.splice(insertIdx + 1, 0, ...newLines);
  }
}
