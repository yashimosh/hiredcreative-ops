 ...
 *
 * Run: node hiredcreative-ops/normalize-statuses.mjs [--dry-run]
 */
...
// Only the evaluated table carries the canonical status column at parts[6].
// The "## Email-confirmed submissions" table puts Cover Letter at parts[6]
// (and uses a different status vocabulary), so blindly normalizing it would
// rewrite that column and corrupt the submission record.
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (parsed.tableOf[i] !== 'evaluated') continue;
